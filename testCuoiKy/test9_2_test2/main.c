...
int insertArrayInArray(int arrMain[], int *sizeMain, int arrSub[], int sizeSub
                        , int pos);
int main()
{
    int arrMain[100];
    ...
    printf("\nCac phan tu arrMain la: ");
    outputArray(arrMain, sizeMain);

    inputArray(arrSub, &sizeSub);
    printf("\nCac phan tu arrSub la: ");
    outputArray(arrSub, sizeSub);

    printf("\nNhap vao pos: ");
    scanf("%d", &pos);

    int tmp = insertArrayInArray(arrMain, &sizeMain, arrSub, sizeSub, pos);
    if(tmp == 0){
        printf("\nVi tri khong hop li");
    }else{
        printf("\nMang sau khi xu li: ");
